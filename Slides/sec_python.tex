...
\begin{frame}{Arrays erstellen}
  \begin{itemize}
    ...
    \item \texttt{np.linspace(start, end, number)}: \texttt{number} Zahlen zwischen \texttt{start} und \texttt{end} in gleichem Abstand
    \item \texttt{np.arange(start, end, step)}: Zahlen zwischen \texttt{start} und \texttt{end} mit dem Abstand \texttt{step}
...
\begin{frame}{Elementweise Funktionen}
  ...
  \begin{itemize}
    \item \texttt{np.sqrt}
    \item \texttt{np.exp}, \texttt{np.log}
    \item \texttt{np.sin}
    \item \texttt{np.deg2rad}, \texttt{rad2deg}
  \end{itemize}
\end{frame}
...
\begin{frame}{Reduzierende Funktionen}
  ...
  \begin{itemize}
    \item \texttt{np.sum}
    \item \texttt{np.mean}
    \item \texttt{np.max}, \texttt{np.min}
    \item \texttt{np.ediff1d}
  \end{itemize}
\end{frame}

\begin{frame}{Input/Output}
  \begin{itemize}
    \item \texttt{np.loadtxt(file [, unpack=True]}: Lädt eine Datei in ein Array.
      \texttt{unpack=True} transponiert das Array
    \item \texttt{np.savetxt(file, array)}: Speichert ein Array in eine Datei
  \end{itemize}
\end{frame}
...
\begin{frame}{Nützliche Funktionen}
  \begin{itemize}
    \item \texttt{scipy.optimize.curve\_fit}: fittet nichtlineare Funktionen
    \item \texttt{scipy.stats.sem}: gibt den Fehler des Mittelwerts
...
\begin{frame}{Konstanten}
  \begin{itemize}
    \item \texttt{constants.physical\_constants}: enthält diverse physikalische Konstanten
  \end{itemize}
\end{frame}
